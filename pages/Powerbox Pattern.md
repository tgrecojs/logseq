...
- ## Overview
	...
	- ### The Role of the Powerbox in Delegating Authority
		...
		- Powersboxes can be dynamic
			- As mentioned above, **the acts of a powerbox passing out authority (mentioned above) is an on-going process.** As an application evo Throughout an application's lifecycle   In some instances, a Powerbox may receive a request for a capability that it does itself not in possession of a capability object that exists within its system requests a reference for a capability that the Powerbox itself does not possess
